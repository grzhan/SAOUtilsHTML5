...
var SAOMainPanel = {
	_new : function() {
		...
		panel.moveTo = function(targetItem) {
			console.log('Panel Moving to ' + targetItem.getID());
		};

		panel.fadeIn = function(targetItem) {
			// $(id).fadeIn();
			console.log('Panel Fading In '+ targetItem.getID() +' !');
		};
...
var SAOMainMenu = {
	_new : function() {
		...
		menu.getStartPosX = function() { return _startPosX;};
		menu.getStartPosY = function() { return _startPosY;};
				...
				if (selected !== indexItem[item_id]) {
					...
					if (panel_dom.css('display') === undefined ||
					  panel_dom.css('display') === 'none') {
						panel.fadeIn(items[indexItem[item_id]]);
					} else {
						panel.moveTo(items[indexItem[item_id]]);
					}
				} else {
					selected = -1;
		...
		menu.fadeIn = function() {
			...
			menu_dom.css('top',mStartPosY - 300);
			menu_items = $('.menu_btn');
